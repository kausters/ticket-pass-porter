...
export function getTickets(data: Line[]): TicketReadData[] {
	return getTicketIndices(data)
		.map(({ start, end }) => data.slice(start, end + 1)) // Include the end row
		.map(parseTicket);
}

function getTicketIndices(data: Line[]): { start: number; end: number }[] {
	...
	const indexOfMarker = (fromIndex: number) =>
		data.findIndex((line, index) => index >= fromIndex && line.str === endMarker);

	const ticketCount = data.filter((line) => line.str === endMarker).length;
	const startIndex = 2;
	...
	ticketRowIndices.push({
		start: startIndex,
		end: indexOfMarker(startIndex) + endOffset,
	});
	...
	for (let i = 1; i < ticketCount; i++) {
		...
		const end = indexOfMarker(start) + endOffset;
